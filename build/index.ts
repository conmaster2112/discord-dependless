...
const outputFileName = "./src/rest-api-types.ts";
let outro = `
...
outro+=`export type ${getName("GatewayEventNames")} = ${gateway_event_names};\n\n`;
const handle = createWriteStream(outputFileName);
const typeMapper = {
    integer: "number",
    float: "number"
}
...
const response = await fetch(API_DATA_KIND);

if(!response.ok) throw console.error("Failed to fetch types.");
...
for(const chunk of writeEndPoints(data.paths)) handle.write(chunk);
for(const chunk of writeSchemas(data.components.schemas)) handle.write(chunk);
...
function getTypeFromType(data: any){
    if("const" in data) {
    ...
    if(!data.type) {
        console.warn("Unknown type: " + JSON.stringify(data));
        return "unknown";
    }
    return getType(data);
}
function getType(e: any): string | undefined{
    return typeMapper[e.type]??e.type;
}
function getName(e: string): string{
    return `Discord${e}`;
}
handle.write(outro);
handle.close();
console.log("Done in ",performance.now() - initTime, "ms");